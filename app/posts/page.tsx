...
function Posts() {
  return (
    <>
      <h1 className="m-8 text-center text-4xl font-bold">Posts</h1>
      <div className="grid grid-cols-1 place-items-center gap-5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {posts.map((post, i) => (
          <Link
            key={i}
            href={`/posts/${post.slug}`}
            className="group w-full max-w-[392px] rounded-lg border border-zinc-200 p-4 transition-all duration-300 ease-in-out dark:border-zinc-800"
          >
            ...
            <h2 className="line-clamp-2 py-1 text-center text-xl font-bold leading-7 md:text-left md:text-2xl">
              {post.title}
            </h2>

            <div className="flex justify-between py-3 text-base text-zinc-500">
              <div>{post.author}</div>
              <div>{post.date}</div>
            </div>
          </Link>
